...
public class MyContextPathBuilder extends ContextPathBuilder {
	...
	public String getUserFilesPath(HttpServletRequest request) {
		...
		if (manage != null && manage.getRole() != null) {
			return domain
					+ PropertiesLoader.getProperty("connector.manageFilesPath")
							.replaceAll("\\{0\\}", manage.getUsername());
		} else if (user != null) {
			return domain
					+ PropertiesLoader.getProperty("connector.userFilesPath")
							.replaceAll("\\{0\\}", user.getUsername());
		}
		return null;
	}
	...
	public String getUserFilesAbsolutePath(HttpServletRequest request) {
		...
		if (manage != null && manage.getRole() != null) {
			return PropertiesLoader.getProperty("connector.manageFilesPath")
					.replaceAll("\\{0\\}", manage.getUsername());
		} else if (user != null) {
	...
	public static Object getSession(HttpServletRequest request, String name) {
		if (request == null || Validator.isBlank(name))
			return null;
