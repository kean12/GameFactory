...
public class RoleLimitAction extends BaseAction {
	...
	public String saveRoleLimits() throws Exception {
		...
		// 格式为
		// '208,208_29,208_30,208_31,208_32,208_33,208_36,208_37,208_38,208_39'
	...
	/*
	 * {id:0, item:[ {id:1,text:"first"}, {id:2, text:"middle", item:[ {id:"21",
	 * text:"child",checked:'true'} ]}, {id:3,text:"last"} ] }
	 */
	public String dhtmlxTreeJSON() throws Exception {
		...
		for (Role r : roleList) {
			buff.append("{id:'" + r.getId() + "',text:'" + r.getDescription()
					+ "',color:'red'");
		...
		for (int i = 0; i < menuList.size(); i++) {
			...
			if (!Validator.isEmpty(m.getChild())) {
				...
				for (Menu chidMenu : set) {
					...
					buff.append("{id:'" + id + "',text:'"
							+ chidMenu.getMenuName() + "'");
					...
					if (!Validator.isEmpty(chidMenu.getChild())) {
						...
						for (Menu chidChildMenu : childSet) {
							id = (int) roleId + "_"
									+ chidChildMenu.getId().intValue();
							buff.append("{id:'" + id + "',text:'"
									+ chidChildMenu.getMenuName()
									+ "',checked:"
									+ chidChildMenu.checkPower(roleId) + "},");
						}
