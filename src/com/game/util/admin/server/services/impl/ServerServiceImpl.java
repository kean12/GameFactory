...
public class ServerServiceImpl extends GenericServiceImpl<Server, Long>
		implements ServerService {

	public Page<Server> searchServerByArea(String serverName, Long areaID,
	...
	public Long getServerMaxID(Long areaID) throws Exception {
		...
		List<Object> list = baseDAO.findList(
				"select max(id) from Server a where a.area.id=?", areaID);
	...
	public Server findServerByName(String serverName, Long areaID)
			throws Exception {
		String hql = "from Server a where a.serverName=? and a.area.id="
				+ areaID;
	...
	public List<Server> findServerByState(Integer state) throws Exception {
		return baseDAO.findEntity(
				"from Server a where a.state=? order by a.id", state);
	}

	public List<Server> findServerByArea(Long areaID, Integer state)
			throws Exception {
		String hql = "from Server a where a.area.id=? and a.state=" + state
				+ " order by a.id";
