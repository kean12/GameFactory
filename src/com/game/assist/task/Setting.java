...
public class Setting {
	 ...
	 */
	protected static String[] invalidName = { "", "true", "false", "yes", "no",
			"server", "client", "enter", "exit", "to" };
	...
	/**
	 * Method isInvalidName 测定一个字符串用户名在系统中是否被认为是合法用户名
	 * 
	 * @param name
	 *            被测试的用户名
	 * 
	...
	/**
	 * Method getAllElements 取得Document中指定一个Element的所有子Element，Element的结构是一个树形结构
	 * 遍历时按深度遍历次序 加入到指定List中。 该方法是用来取得Document的内部属性结构。
	 * 
	 * @param list
	 *            盛放取得的Element序列的有序容器
	 * @param Element
	...
	/**
	 * Method createButton 按给定的参数构造一个JButton组件
	 * 
	 * @param text
	 *            button组件显示的text
	 * @param mn
	 *            指定的Mnemonic 其精度就小于等于char的精度
	 * @param command
	 *            button组件的actionCommand
	 * @param stroke
	 *            button组件对应的快捷键，其有效应用范围是WHEN_IN_FOCUSED_WINDOW
	 * @param Container
	 *            button组件被加入的容器
	 * @param als
